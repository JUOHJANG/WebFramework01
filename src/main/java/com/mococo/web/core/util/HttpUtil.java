package com.mococo.web.core.util;
...
public class HttpUtil {
	...
	public boolean writeResponseFileStream(String fileName, HttpServletResponse response) {
		...
		try {
			...
			if (fileInputStream != null) { try { fileInputStream.close(); } catch (Exception e) { logger.error("!!! error", e); } }
			if (bufferedInputStream != null) { try { bufferedInputStream.close(); } catch (Exception e) { logger.error("!!! error", e); } }
			if (bufferedOutputStream != null) { try { bufferedOutputStream.close(); } catch (Exception e) { logger.error("!!! error", e); } }
			if (servletOutputStream != null) { try { servletOutputStream.close(); } catch (Exception e) { logger.error("!!! error", e); } }
		}			
	...
	public static boolean isJsonRequest(HttpServletRequest request) {
		return request.getContentType().matches("application/json.*|application/javascript.*") || "XMLHttpRequest".equalsIgnoreCase(request.getHeader("X-Requested-With"));
	}
}