...
public class MstrExpressionPromptApiDao extends MstrPromptDao<WebExpressionPrompt> implements ClientResponse<Map<String, Object>, List<PromptElement>> {
	...
	public MstrExpressionPromptApiDao(WebIServerSession session, WebExpressionPrompt prompt) {
        ...
        if (((WebExpressionPrompt)prompt).getExpressionType() == EnumDSSXMLExpressionType.DssXmlFilterAttributeIDQual) {
	...
	public List<PromptElement> getDefaultAnswers() {
		...
		for (int i = 0 ; defaultAnswers != null && i < defaultAnswers.getRootNode().getChildCount(); i ++) {
			...
			DssXmlExpressionCanceledPrompt          24    
			DssXmlExpressionElementList             25    
			DssXmlExpressionElementSingle           26    
			DssXmlExpressionGeneric                  1    Specifies a generic expression type.
			DssXmlExpressionMDXSAPVariable          22    Specifies an expression prompt supporting SAP Variables
			DssXmlExpressionReserved                 0    
			DssXmlExpressionSQLQueryQual            23    
			DssXmlFilterAllAttributeQual            16    Specifies an all attribute qualification
			DssXmlFilterAttributeDESCQual           18    Specifies an attribute description qualification
			DssXmlFilterAttributeIDQual             17    Specifies an attribute ID qualification
			DssXmlFilterBranchQual                  14    Specifies a branch qualification.
			DssXmlFilterEmbedQual                   13    Specifies an embedded qualification.
			DssXmlFilterJointFormQual                4    Specifies a joint form qualification
			DssXmlFilterJointListFormQual            8    Specifies a joint list qualification involving attribute forms
			DssXmlFilterJointListQual                7    Specifies a joint list qualification
			DssXmlFilterListFormQual                 6    Specifies a list qualification involving an attribute form (example: Customer(Last Name) In ("Jacobson", "Jones")
			DssXmlFilterListQual                     5    Specifies a list qualification.
			DssXmlFilterMetricExpression            12    Specifies a metric expression qualification (example: M1 > M2).
			DssXmlFilterMultiBaseFormQual            3    Specifies a multy base form qualification
			DssXmlFilterMultiMetricQual             11    Specifies a qualification on multiple metrics.
			DssXmlFilterRelationshipQual            15    Specifies a relationship qualification
			DssXmlFilterReportQual                  21    Specifies a filter report qualification
			DssXmlFilterSingleBaseFormExpression     9    Specifies a single base form expression qualification.
			DssXmlFilterSingleBaseFormQual           2    Specifies a single base form qualification (example: Customer(Last Name) Like "C*").
			DssXmlFilterSingleMetricQual            10    Specifies a single metric qualification (example: Sales > 100).
			*/
			...
			if (expressionType != EnumDSSXMLExpressionType.DssXmlFilterListQual) { continue; }
		...
		public static List<WebDimensionAttribute> getDimensionAttributeList(WebPrompt paramPrompt) {
			...
			try {
				...
				for (Enumeration<WebDimension> e = units.elements(); e.hasMoreElements(); ) {
					...
					dimension.populate();
					for (Enumeration<WebDimensionAttribute> e1 = dimension.getTopLevelAttributes().elements(); e1.hasMoreElements(); ) {
						WebDimensionAttribute dimensionAttribute = e1.nextElement();
						
						if (dimensionAttribute.getParents().size() != 0) { continue; }
						
						logger.debug("*** top attribute -> id: [{}], displayName: [{}]", dimensionAttribute.getID(), dimensionAttribute.getDisplayName());
	...
	public List<PromptElement> getSuggestedAnswers() {
		...
		public List<PromptElement> getFilteredElements(WebDimensionAttribute dimensionAttribute, String attributeId, String filterElementId) throws WebObjectsException, IllegalArgumentException {
			...
			WebOperatorNode inlistNode = expression.createOperatorNode(EnumDSSXMLExpressionType.DssXmlFilterListQual,  EnumDSSXMLFunction.DssXmlFunctionIn, root);
			WebAttribute filterAttribute = (WebAttribute)source.getObject(attributeId, EnumDSSXMLObjectTypes.DssXmlTypeAttribute);
		...
		public List<PromptElement> getLevelElements(int parentLevel, String parentSelectedElementId) throws WebObjectsException, IllegalArgumentException {
			...
			List<PromptElement> elements = getFilteredElements(childDimensionAttribute, parentDimensionAttribute.getAttribute().getID(), parentSelectedElementId);
