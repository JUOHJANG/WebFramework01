...
public class ConfigManager {
	...
	/**
	 * 싱글톤으로 구현되어 클래스 로딩 시 1회 수행된다.
	 * sdk 환경설정 파일에서 구현클래스명/파라미터를 조회하여 이를 이용하여 인스턴스틀 생성하고, 초기화 메소스 initConfigObjectList를 호출한다. 
	 * 
	 * sdk 환경설정 파일 중 configHandler의 구현클래스명
	 * 해당 클래스의 인스턴스를 생성할때 전달될 파라미터를 기재한다.
	{ ...
	"configHandler":{
		"config1":{
			"className": "com.mocomsys.microstrategy.sdk.config.impl.JsonConfigHandler",
			"param":{
				"filePath":"mstr-sdk-prompt-config.json"
			}
		},
		"config2":{
			"className": "com.mocomsys.microstrategy.sdk.config.impl.JsonConfigHandler",
	...
	private ConfigManager() { 
		...
		try {
			Map<String, Map<String, Object>> configHandlerConfig = PropManager.<Map<String, Object>>getProp("configHandler"); 
