package com.mococo.microstrategy.sdk.core.util;
...
import com.mococo.biz.demo.controller.MstrController;
...
public class MstrReportUtil {
	...
	/**
	 * Example:
  	 * 		WebReportInstance reportInstance = getFinishedReportInstance(
     *			session, objectId, EnumDSSXMLExecutionFlags.DssXmlExecutionNoAction, EnumDSSXMLResultFlags.DssXmlResultDtlsFilterExpr);
	 * 사용용도:
	 *  	일반 리포스 실행
	 *  		- executionFlags = -1, resultFlags = -1 로 지정  
  	 *		리포트의 필터객체를 조회할때 사용
  	 *			- EnumDSSXMLExecutionFlags.DssXmlExecutionNoAction, EnumDSSXMLResultFlags.DssXmlResultDtlsFilterExpr 로 지정
  	 *  	리포트 익스포트 실행 
  	 *  		- EnumDSSXMLResultFlags.DssXmlResultGrid | EnumDSSXMLResultFlags.DssXmlResultBandingStyle
	 * 
	 * @param session
	 * @param objectId
	...
	public static WebReportInstance getFinishedReportInstance(WebIServerSession session, String objectId,
		...
		while (true) {
			if (hasFinishedInformation(reportInstance.pollStatus())) { break; }
			Thread.sleep(200);
		}
	...
	public static WebDocumentInstance getFinishedDocumentInstance(WebIServerSession session, String objectId,
		...
		while (true) {
			if (hasFinishedInformation(documentInstance.pollStatus())) { break; }
			Thread.sleep(200);
		}
		return documentInstance;
	}
	
	public static String getReportAnswerXML(WebIServerSession session, String objectId, int type, Map<String, List<String>> param) throws WebObjectsException, InterruptedException {
		String result = null;
	...
	/*
	 * date : 2022-04-25
	 * 선택된 개체 가져온 후, 리포트 저장 메소드
	 * */
	public static String ExecuteReport(WebIServerSession session, ArrayList<String> attribute,ArrayList<String> metric, ArrayList<String> prompt, int executionFlags, int resultFlags , String reportName) throws WebObjectsException, InterruptedException{
        ...
        reportSource.setResultFlags(EnumDSSXMLResultFlags.DssXmlResultWorkingSet | EnumDSSXMLResultFlags.DssXmlResultViewReport | EnumDSSXMLResultFlags.DssXmlResultStatusOnlyIfNotReady);
...
//	                            elements.add(PromptElements.get(0).getID().toString()); 
	            ...
	            newInst.setSaveAsFlags(EnumDSSXMLReportSaveAsFlags.DssXmlReportSaveAsFilterWithPrompts | EnumDSSXMLReportSaveAsFlags.DssXmlReportSaveAs8iWarning | EnumDSSXMLReportSaveAsFlags.DssXmlReportSaveAsEmbedded);
...
//	            String datedReportName = reportName;
    ...
    private static void createElementsPrompt(WebIServerSession session,WebExpression expression, WebObjectInfo objectinfo) throws WebObjectsException {
		WebObjectsFactory objectFactory = session.getFactory();
		 WebObjectSource objectSource = objectFactory.getObjectSource();
        //Retrieve a new Elements Prompt
        WebElementsPrompt elementsPrompt = (WebElementsPrompt)objectSource.getNewObject(EnumDSSXMLObjectTypes.DssXmlTypePrompt, EnumDSSXMLObjectSubTypes.DssXmlSubTypePromptElements);
        ...
        WebOperatorNode weboperatornode = expression.createOperatorNode(EnumDSSXMLExpressionType.DssXmlExpressionGeneric,EnumDSSXMLFunction.DssXmlFunctionIn, expression.getRootNode());
					...
					if (answerObjectIdList != null) {
						for (String answerObjectId : answerObjectIdList) {
							...
							for (Enumeration<?> e2 = suggested.elements(); e2.hasMoreElements(); ) {
								...
								if (StringUtils.equals(info.getID(), answerObjectId)) { continue; }
