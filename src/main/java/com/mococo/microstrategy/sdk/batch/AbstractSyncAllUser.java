...
public abstract class AbstractSyncAllUser {
	...
	private void setTargetUserList() {targetUserList = getTargetUserInfo();}
	protected List<UserInfo> getTargetUserList() { return targetUserList; }
	
	protected Map<String, String> getMstrGroupMap() { return mstrGroupMap; }
	protected Map<String, String> getMstrUserMap() { return mstrUserMap; }
	...
	/* 
		파라미터로 전달된 user가 소속된 모든 사용자그룹의 목록을 반환
		(mstrGroupMap에 포함된 처리대상 사용자그룹만 식별)
	*/
			...
			for (UserInfo targetUser : targetUserList) {
				try {
					...
					if (StringUtils.isEmpty(objectId)) {
						...
						// 새로운 사용자 생성
						user = MstrUserUtil.createUserWithMail(session, targetUser.getUserId(), targetUser.getUserName(), null, targetUser.getEmail(), getMailDeviceId());
						objectId = user.getID();
	...
	public synchronized static void jobStart() { active = true; logger.debug("=> 작업을 시작합니다."); }
	public synchronized static void jobEnd() { active = false; logger.debug("=> 작업을 종료합니다."); }
