...
public class CustomExternalSecurity extends AbstractExternalSecurity {
    ...
    public WebIServerSession getWebIServerSession(final RequestKeys reqKeys, final ContainerServices cntSvcs) {
        ...
        try {
            ...
            if (StringUtils.isEqual(sessionState, newSessionState)) { setSessionState(cntSvcs, project, newSessionState); }
	...
	// 세션의 생성
	private boolean connectISession(ContainerServices cntSvcs, String server, String project, int port, String mstrUserId) {
        try {
            MstrUser user = getMstrUser(cntSvcs);
            
            WebIServerSession session = MstrUtil.connectSession(server, port, project, mstrUserId, user.getPassword(), user.getAuthMode(), getLocale(), getTrustToken(), getClientID(cntSvcs));
	...
	protected String getServer(RequestKeys reqKeys) {
		...
		if (StringUtils.isEmpty(server)) { server = CustomProperties.getProperty("mstr.default.server"); }
		return server;
	}
	...
	protected int getPort(RequestKeys reqKeys) {
		...
		if (port == 0) {
			try {
				port = Integer.parseInt(CustomProperties.getProperty("mstr.default.port"));
			} catch (Exception e) {
				port = 0;
			}
		}
	...
	protected String getProject(RequestKeys reqKeys) { 
		...
		if (StringUtils.isEmpty(project)) { project = CustomProperties.getProperty("mstr.default.project"); }
		
		return project;  
	}
	
	private String getTrustToken() { return CustomProperties.getProperty("mstr.trust.token"); }
	private String getEsmFailUrl() { return CustomProperties.getProperty("mstr.esm.fail.url"); }
	private int getLocale() {
		String token = CustomProperties.getProperty("mstr.session.locale");
		int locale = -1;
		
		try {
			locale = Integer.parseInt(token);
		} catch (Exception e) {
			locale = 1033;
		}
	...
	private void setSessionState(ContainerServices cntSvcs, String project, String sessionState) {
		MstrUser mstrUser = getMstrUser(cntSvcs);
		if (mstrUser != null) { mstrUser.setProjectSession(StringUtils.isEmpty(project) ? "-" : project, sessionState); }
	}
